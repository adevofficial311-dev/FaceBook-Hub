...
-- Per-user file storage: each Roblox account (by UserId) gets its own
-- settings file, so saved values never get mixed up between users on
-- the same device/executor. Falls back gracefully if file IO isn't available.
local canUseFiles = typeof(writefile) == "function"
	and typeof(readfile) == "function"
	and typeof(isfile) == "function"

local CONFIG_FOLDER = "MatizHub"
local CONFIG_FILE = CONFIG_FOLDER.."/MotionBlur_"..tostring(player.UserId)..".json"

if canUseFiles and typeof(makefolder) == "function" and typeof(isfolder) == "function" then
	pcall(function()
...
local function loadConfig()
	...
	if canUseFiles then
		pcall(function()
			if isfile(CONFIG_FILE) then
				...
				if typeof(data.enabled) == "boolean" then config.enabled = data.enabled end
				if typeof(data.mode) == "string" and (
					data.mode == "Camera" or data.mode == "Velocity" or
					data.mode == "Acceleration" or data.mode == "Hybrid"
				) then config.mode = data.mode end
				if typeof(data.intensity) == "number" then config.intensity = math.clamp(data.intensity, 0, 1) end
				if typeof(data.cameraEnabled) == "boolean" then config.cameraEnabled = data.cameraEnabled end
				if typeof(data.velocityEnabled) == "boolean" then config.velocityEnabled = data.velocityEnabled end
				if typeof(data.accelerationEnabled) == "boolean" then config.accelerationEnabled = data.accelerationEnabled end
				if typeof(data.cameraWeight) == "number" then config.cameraWeight = math.clamp(data.cameraWeight, 0, 1) end
				if typeof(data.velocityWeight) == "number" then config.velocityWeight = math.clamp(data.velocityWeight, 0, 1) end
				if typeof(data.accelerationWeight) == "number" then config.accelerationWeight = math.clamp(data.accelerationWeight, 0, 1) end
	...
	if loadedFromFile then return end
