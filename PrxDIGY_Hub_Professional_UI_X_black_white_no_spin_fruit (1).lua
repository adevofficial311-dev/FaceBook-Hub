...
local hopping = false
local failedServers = {}
local currentTargetServer = nil
local hopQueued = false

-- Kept for compatibility with the rest of the script, but Auto Server Hop
-- does NOT call this. Server hopping should not reset the character.
...
local function getPublicServerPages()
    local allServers = {}
    local cursor = nil

    -- Pull several pages so a stale/restricted server on the first page does
    -- not trap the hopper. The endpoint is explicitly the PUBLIC server list.
    for _ = 1, 5 do
...
local function buildServerCandidates()
    local servers = getPublicServerPages()
    local candidates = {}
    local seen = {}
    ...
    for _, server in ipairs(servers) do
        ...
        -- Only accept valid, non-full PUBLIC instances. If Roblox supplies a
        -- privateServerId field, require it to be empty as an extra safeguard.
        local isPrivate = server.privateServerId and server.privateServerId ~= ""
        local isReserved = server.reservedServerAccessCode and server.reservedServerAccessCode ~= ""
        ...
        if type(id) == "string"
            ...
            and not isPrivate
            and not isReserved then
            seen[id] = true
            table.insert(candidates, server)
        end
    end

    -- Randomize the eligible public servers so we don't repeatedly hit the
    -- same stale server ordering returned by the API.
...
local function attemptNextPublicServer()
    ...
    local candidates = buildServerCandidates()

    -- If every known server has failed, clear the blacklist and get a fresh
    -- public-server snapshot instead of retrying the same stale target.
    if #candidates == 0 then
        failedServers = {}
        candidates = buildServerCandidates()
    end
    ...
    failedServers[server.id] = true
    currentTargetServer = server.id
    ...
    if not success then
        -- The call itself failed, so immediately move to another public server.
        failedServers[server.id] = true
        currentTargetServer = nil
        task.wait(0.25)
        return attemptNextPublicServer()
    end

    -- A successful pcall only means Roblox accepted the teleport request.
    -- TeleportInitFailed below handles a later Restricted/Unavailable failure.
    return true
end

TeleportService.TeleportInitFailed:Connect(function(player, teleportResult, errorMessage, placeId)
    if player ~= LocalPlayer or placeId ~= game.PlaceId or not hopping then
        return
    end

    -- Never retry the server that Roblox just rejected.
    if currentTargetServer then
        failedServers[currentTargetServer] = true
    end
    currentTargetServer = nil
    ...
    notify("Server Hop", "Server rejected. Trying another public server...", 2)

    if not hopQueued then
        hopQueued = true
        task.delay(0.5, function()
            hopQueued = false
            if AutoServerHop and hopping then
                attemptNextPublicServer()
            elseif not AutoServerHop then
                hopping = false
            end
        end)
    end
...
local function teleportToNextServer()
    ...
    local success = attemptNextPublicServer()
    if not success then
        hopping = false
        return false
    end
...
task.spawn(function()
    while task.wait(1) do
        if not AutoServerHop then
            ...
            hopping = false
            currentTargetServer = nil
        elseif not hopping and #getFruitObjects() == 0 then
            emptyScans += 1
            if emptyScans >= 1 then
                ...
                if #getFruitObjects() == 0 then
                    notify("Server Hop", "Trying a different public server...", 2)
                    if not teleportToNextServer() then
                        notify("Server Hop", "No usable public server found. Retrying with a fresh list...", 3)
                    end
                end
            end
        else
