...
floating.Position = UDim2.new(1,-76,0.5,-28)
...
panel.Size = UDim2.fromOffset(350,455)
...
toggleText.Size = UDim2.new(1,-85,1,0)
...
modeCard.Size = UDim2.new(1,0,0,94)
...
mcorner.CornerRadius = UDim.new(0,11)
mcorner.Parent = modeCard

local modeTitle = Instance.new("TextLabel")
modeTitle.Size = UDim2.new(1,-30,0,24)
modeTitle.Position = UDim2.fromOffset(15,8)
...
hint.Position = UDim2.fromOffset(15,28)
hint.BackgroundTransparency = 1
hint.Text = "Hybrid combines camera, velocity and acceleration."
...
row.Position = UDim2.fromOffset(15,52)
...
for _,name in ipairs(modes) do
	...
	b.MouseButton1Click:Connect(function()
		config.mode = name
		saveConfig()
		for n,x in pairs(modeButtons) do
			x.BackgroundColor3 = (n == config.mode) and C.accent2 or C.surface3
			x.TextColor3 = (n == config.mode) and C.white or C.sub
		end
	end)
end
...
floating.MouseButton1Click:Connect(function()
	...
	if panel.Visible then
		panel.Size = UDim2.fromOffset(330,425)
		tw(panel,.22,{Size=UDim2.fromOffset(350,455)})
	end
end)
...
toggle:GetPropertyChangedSignal("Visible"):Connect(function() refreshUI() end)
...
RunService.RenderStepped:Connect(function(dt)
	...
	elseif config.mode == "Acceleration" then
		...
		-- Hybrid = all three systems working together.
		raw = math.clamp(camAmount + velAmount + accAmount,0,1)
	end
...
task.defer(function()
	...
	if cam and cam.ViewportSize.X < 600 then
		floating.Position = UDim2.new(1,-66,.72,0)
	end
end)
